...
def main():
    ...
    high = load("ai/high-risk-boundary-policy.json")
    sources = load("sources/public-sources.json")
    evidence = load("sources/evidence-sources.json")
    ...
    require(b["load_order"] == required, "policy load order changed")
    for needed in ("sources/public-sources.json", "profiles/learning-accessibility.json", "examples/index.json"):
        require(needed in b["routed_records"]["cbt_self_help"], f"self-help route missing {needed}")
    for route in ("medical_claim", "clinical_guideline", "evidence_audit"):
        for needed in (
            ...
            require(needed in b["routed_records"][route], f"{route} route missing {needed}")
    for route in ("high_risk_boundary", "urgent_safety"):
        require("ai/high-risk-boundary-policy.json" in b["routed_records"][route], f"{route} route missing high-risk boundary policy")
        require("ai/safety-escalation-policy.json" in b["routed_records"][route], f"{route} route missing safety escalation policy")
    ...
    for guard in {
        ...
        require(guard in set(boundary["hard_guards"]), f"missing guard {guard}")
    for guard in {
        ...
        require(guard in set(high["hard_guards"]), f"missing high-risk guard {guard}")

    require(set(high["contexts"]) == {"trauma", "possible_psychosis", "possible_mania", "eating_disorder", "substance_use"}, "high-risk context set changed")
    require(high["urgent_safety_precedence"] == "ai/safety-escalation-policy.json", "high-risk policy must defer to urgent safety policy")
    require(epi["fallback_state"] == "unknown", "epistemic contract must fail to unknown")
    require(safety["principle"] == "urgent safety needs override routine CBT exercises", "safety override weakened")
    require("move emotion into the prefrontal cortex" in work["avoid_framing"], "neuroscience cartoon guard missing")
    require(work.get("evidence_scope") and work.get("mechanism_summary") and work.get("guardrails"), "cognitive-work guardrails incomplete")

    records = sources["sources"] + evidence["sources"]
    source_ids = {s["id"] for s in records}
    require(len(source_ids) == len(records), "duplicate source id across registries")
    for source in sources["sources"]:
        if source.get("class") == "peer_reviewed_pubmed":
    ...
    require(ids == set(access["plain_language_exercises"]), "plain-language variants must cover every exercise")
    ...
    eval_errors = validate_suite(ROOT)
    require(not eval_errors, "adversarial safety suite invalid: " + "; ".join(eval_errors))

    interface = b.get("evaluation_interfaces", {}).get("adversarial_safety", {})
    expected_interface = {
        "canonical": False,
        "suite": "evals/adversarial-safety/cases.json",
        "invariants": "evals/adversarial-safety/invariants.json",
        "response_schema": "evals/adversarial-safety/response-envelope.schema.json",
        "runner": "tools/adversarial_safety.py",
    }
    require(interface == expected_interface, "adversarial evaluation interface paths or canonicality changed")
